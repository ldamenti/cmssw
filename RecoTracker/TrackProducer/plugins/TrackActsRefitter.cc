...
class TrackActsRefitter : public KfTrackProducerBase, public edm::stream::EDProducer<> {
public:
  /// Constructor
  explicit TrackActsRefitter(const edm::ParameterSet &iConfig);
...
private:
  TrackProducerActsAlgorithm<reco::Track> theAlgo;
...
TrackActsRefitter::TrackActsRefitter(const edm::ParameterSet &iConfig)
...
void TrackActsRefitter::fillDescriptions(edm::ConfigurationDescriptions &descriptions) {
  ...
  TrackProducerActsAlgorithm<reco::Track>::fillPSetDescription(desc);
  KfTrackProducerBase::fillPSetDescription(desc);
  descriptions.addWithDefaultLabel(desc);
}

void TrackActsRefitter::produce(edm::Event &theEvent, const edm::EventSetup &setup) {
  LogDebug("TrackActsRefitter") << "Analyzing event number: " << theEvent.id() << "\n";
  //
  ...
  switch (constraint_) {
    case none: {
      ...
      LogDebug("TrackActsRefitter") << "TrackActsRefitter::produce(none):Number of Trajectories:" << (*theTCollection).size();

      if (bs.position() == math::XYZPoint(0., 0., 0.) && bs.type() == reco::BeamSpot::Unknown) {
        edm::LogError("TrackActsRefitter") << " BeamSpot is (0,0,0), it is probably because is not valid in the event";
        break;
      }
      ...
      if (theTCollection.failedToGet()) {
        ...
        edm::LogError("TrackActsRefitter") << "could not get the reco::TrackCollection." << labels.module;
        break;
      }
      LogDebug("TrackActsRefitter") << "run the algorithm"
                                << "\n";
    ...
    case momentum: {
      ...
      if (theTCollectionWithConstraint.failedToGet()) {
        //edm::LogError("TrackActsRefitter")<<"could not get TrackMomConstraintAssociationCollection product.";
        break;
      }
      LogDebug("TrackActsRefitter") << "run the algorithm"
                                << "\n";
    ...
    case vertex: {
      ...
      bs = *recoBeamSpotHandle;
      if (theTCollectionWithConstraint.failedToGet()) {
        edm::LogError("TrackActsRefitter") << "could not get TrackVtxConstraintAssociationCollection product.";
        break;
      }
      LogDebug("TrackActsRefitter") << "run the algorithm"
                                << "\n";
    ...
    case trackParameters: {
      ...
      bs = *recoBeamSpotHandle;
      if (theTCollectionWithConstraint.failedToGet()) {
        //edm::LogError("TrackActsRefitter")<<"could not get TrackParamConstraintAssociationCollection product.";
        break;
      }
      LogDebug("TrackActsRefitter") << "run the algorithm"
                                << "\n";
  ...
  putInEvt(theEvent,
           ...
           theBuilder.product(),
           &ttopo);
  LogDebug("TrackActsRefitter") << "end"
                            << "\n";
}

#include "FWCore/Framework/interface/MakerMacros.h"
DEFINE_FWK_MODULE(TrackActsRefitter);
